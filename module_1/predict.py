...
parser.add_argument('--x_test_dir', help='Path to X_test.')
parser.add_argument('--y_test_dir', help='Path to y_test.')
parser.add_argument('--model_input_dir', help='Path to godlike model.')
...
### Code
if (args.x_test_dir is None or \
	args.y_test_dir is None or \
	args.model_input_dir is None):
	print '\n' \
		  'Specify test data and model weights!\n'
	sys.exit()
...
# kernelize test data
if not os.path.exists('batches_test'):
	if (args.x_train_dir is None or args.y_train_dir is None):
		print '\n' \
			  'No kernel data detected!\n' \
			  'Specify --x_train_dir and --y_train_dir.\n' \
			  '\n' \
			  'Abort.'
		sys.exit()
	...
	kernelize(X_test, X[landmarks_idx], 'test')

# Load kernel test data
batches_test = load_batches('test')

# Load weights
with open(args.model_input_dir, 'rb') as fp:
	W = np.load(fp)

predicted_labels = predict_labels(batches_test, W)
